    ...
    if (activeScanType === "image") {
      ...
      reader.onload = async function (e) {
        try {
          ...
          // 🔥 ALWAYS SAFE CHECK (zero fail backend compatible)
          const ai = data.ai || {};
          ...
          result.innerHTML = `
            ...
            🔍 Breakdown:<br>
            • Nudity: ${breakdown.nudity ?? 0}<br>
            • Weapon: ${breakdown.weapon ?? 0}<br>
            • Gore: ${breakdown.gore ?? 0}<br>
            • Offensive: ${breakdown.offensive ?? 0}<br>
          `;

        } catch (err) {
          result.innerHTML = "❌ Scan error: " + err.message;
        } finally {
          scanBtn.disabled = false;
        }
      };
