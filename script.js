...
async function startScan() {
  ...
  // Tunazima button ili kuzuia double clicking
  ...
  try {
    ...
    // HAPA NDIPO TULIPOREKEBISHA ILI ISILETE [object Object]
    if (!res.ok || data.error) {
      const errorText = (data.error && typeof data.error === 'object') 
                        ? JSON.stringify(data.error) 
                        : (data.error || "Hitilafu ya Server");
      
      result.innerHTML = `<b style="color:red;">Kosa:</b> ${errorText}`;
    } else {
      // Data ikifika salama
      const ai = data.ai || {};
      const nudity = ai.nudity_2_1?.safe ? (ai.nudity_2_1.safe * 100).toFixed(0) : 0;
      const weapon = ai.weapon_1_0?.prob ? (ai.weapon_1_0.prob * 100).toFixed(0) : 0;
      
      result.innerHTML = `
        <div style="margin-top: 10px;">
          <b>Scan Imekamilika!</b><br>
          Nudity Safe Score: ${nudity}%<br>
          Weapon Risk Score: ${weapon}%
        </div>
      `;
    }
  } catch (err) {
