  ...
  if (activeScanType === "image") {
    ...
    reader.onload = async function (e) {
      try {
        ...
        if (!res.ok) {
          result.innerHTML = "❌ Server error: " + (data.message || "Unknown");
          return;
        }

        const ai = data.ai || {};
const nudity = ai.nudity || {};
const violence = ai.weapon || ai.violence || {};
        result.innerHTML = `
          <b>Image Scan Result</b><br><br>
          Status: ${ai.status || "unknown"}<br>
          Error: ${ai.error?.message || "none"}<br><br>
          Nudity Safe Score: ${nudity.safe ?? "N/A"}<br>
Partial: ${nudity.partial ?? "N/A"}<br>
Violence: ${violence.prob ?? violence.weapon ?? "N/A"}<br>
        `;
      } catch (err) {
        result.innerHTML = "❌ Error processing request: " + err.message;
      }
    };
    ...
    if (!textVal.trim()) {
      result.innerHTML = "⚠️ Please enter some text first";
      return;
    }
    ...
    try {
      const res = await fetch("https://tnet-app.vercel.app/api/scan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: textVal }) // Tuma text, sio image
      });
      ...
      result.innerHTML = "❌ Error processing text scan: " + err.message;
    }
  }
}
