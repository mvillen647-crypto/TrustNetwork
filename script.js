...
async function startScan() {
  ...
  try {
    ...
    // 2. Kutuma kwenye Backend (scan.js mpya)
    ...
    // 3. Kuelewa jibu (Hapa ndipo inapotakiwa "kuelewa")
    if (data.error) {
      result.innerHTML = `<b style="color:red;">Kosa:</b> ${data.error}`;
    } else {
      // Hapa tunasoma data kutoka kwenye SDK mpya
      const ai = data.ai;
      
      // Tunachukua data za Nudity na Weapons kwa mfano
      const nudity = ai.nudity_2_1?.safe || 0;
      const weapon = ai.weapon_1_0?.prob || 0;
      
      result.innerHTML = `
        <b>Scan Imekamilika!</b><br>
        Nudity Safe: ${Math.round(nudity * 100)}%<br>
        Weapon Risk: ${Math.round(weapon * 100)}%
      `;
    }
  } catch (err) {
